 ...
 * 
 * Merged version: stable base + NPC refs, style refs, API presets, avatar preview
 */
...
const defaultSettings = Object.freeze({
    ...
    userAvatarFile: '',
    userCharacterName: '', // Name of user's character as it appears in prompts
    // Style preset
    defaultStyle: '',
...
/**
 * Check if a character name appears in the prompt (fuzzy match)
 */
function nameAppearsInPrompt(name, prompt) {
    ...
    // Fuzzy: try trimming last 1-2 chars for morphological variants
...
/**
 * Get the last generated image from chat as base64 (compressed)
 */
 ...
 * Enhanced with character/style reference separation
 */
async function generateImageGemini(prompt, style, referenceImages = [], options = {}) {
    ...
    // Pre-instruction if we have references
    ...
    // Add character reference images with detailed instructions
 ...
 * Enhanced with labeled references, NPC refs, style refs
 */
async function generateImageWithRetry(prompt, style, onStatusUpdate, options = {}) {
    ...
    // Collect reference images with labels
...
async function processMessageTags(messageId) {
    ...
    // Prevent re-entry after DOM changes
    ...
    // Mark as processed BEFORE saveChat to prevent re-entry
    processedMessages.add(messageId);
    
    await context.saveChat();
    processingMessages.delete(messageId);
    
    // NOTE: Do NOT call messageFormatting here - it causes infinite recursion
...
async function regenerateMessageImages(messageId) {
    ...
    // Remove from processedMessages so it can be marked again after regen
        ...
        <div class="inline-drawer">
            ...
            <div class="inline-drawer-content">
                <div class="iig-settings">
                    ...
                    <!-- User Avatar Selection with Preview Dropdown -->
...
function bindSettingsEvents() {
    ...
    document.getElementById('iig_send_user_avatar')?.addEventListener('change', (e) => {
        ...
        if (nameRow) nameRow.classList.toggle('hidden', !e.target.checked);
        if (nameHint) nameHint.classList.toggle('hidden', !e.target.checked);
    });
